...
function initGalaxyMap() {
    ...
    renderer.setClearColor(0x000033); // Dark blue for debugging
    ...
    const starTexture = loader.load(
        ...
        undefined,
        (err) => console.error("Star texture load error:", err)
    );
    const starMaterial = new THREE.PointsMaterial({ map: starTexture, transparent: true, size: 0.05 });
    const starVertices = [];
    ...
    for (let i = 0; i < numLevels; i++) {
        ...
        const planetTexture = loader.load(
            'assets/textures/planet.jpg',
            () => console.log("Planet texture loaded"),
            undefined,
            (err) => console.error("Planet texture load error:", err)
        );
        const color = getLevelColor(i);
        const material = new THREE.MeshBasicMaterial({ map: planetTexture, color: color });
    ...
    const dustTexture = loader.load(
        'assets/textures/star.png',
        () => console.log("Dust texture loaded"),
        undefined,
        (err) => console.error("Dust texture load error:", err)
    );
    const dustMaterial = new THREE.PointsMaterial({ map: dustTexture, transparent: true, size: 0.02, opacity: 0.5 });
    const dustVertices = [];
